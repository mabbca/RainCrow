...
export function parseWeather(times, weatherResults, parsedWeather) {
  parsedWeather = { 
    ...
    sunrise: null,
    sunset: null,
    attr: `Weather generated by <a href="#" target="_blank">eBird Weather</a>` };
  ...
  for (let icon of icons) {
    parsedWeather.icon += `<img src="http://openweathermap.org/img/wn/${icon}.png" alt="Open Weather Icon" loading="lazy">`; 
  }
...
export async function getWeather(times, checklistInfo, weatherResults) {
  ...
  weatherResults.start = await queryOpenWeather(times.start.utcTime.format("X"), checklistInfo.lat, checklistInfo.lon);
  if (checklistInfo.endTime) {
    console.log("End time weather query for " + times.end.utcTime.local().format("YYYY-MM-DD h:mma Z"));
    weatherResults.end = await queryOpenWeather(times.end.utcTime.format("X"), checklistInfo.lat, checklistInfo.lon);
  }
...
export function getUnixTimes(times, checklistInfo) {
  ...
  console.log('utcTime start after calculation')
  console.log(times.start.utcTime);
  if (checklistInfo.endTime) {
